...
def main() -> None:
    ...
    lines = [
        ...
        f"生成时间（UTC）：`{utc_now()}`",
        "",
        "主口径：batch=1、固定 400×400、仅统计 `session.run()`；能耗为 Intel PCM CPU Package 能量。",
        "",
    ]
    summary = {"providers": {}}
    for provider in args.providers:
        ...
        energy_path = RESULTS_DIR / f"energy_{provider}.json"
        ...
        if energy_path.is_file():
            ...
            provider_summary["energy"] = energy["comparison"]
            lines.extend(
                [
            ...
            for label in ("fp32", "w8a8"):
                ...
                if row.get("valid_trials", 0):
                    ...
                    lines.append(
                        f"| {label.upper()} | {package_energy:.6f} | {dynamic_energy:.6f} | {package_power:.3f} | {stable_text} |"
                    )
                else:
                    lines.append(f"| {label.upper()} | N/A | N/A | N/A | 否（无有效试验） |")
            reduction = (energy.get("comparison") or {}).get("w8a8_package_energy_reduction_fraction")
            lines.extend(["", f"W8A8 Package 能耗变化：`{percent(reduction)}`（正值表示降低）。", ""])
        else:
            lines.extend(["尚未生成 PCM 能耗结果。", ""])
