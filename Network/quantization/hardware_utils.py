...
if Using_MODE == "vcs":
    SER = None
elif Using_MODE == "uart":
    import global_state
    SER = global_state.SER
else:
    raise AssertionError("Using_MODE setting error!")
...
def FC_hardware_compute(input:torch.tensor, layer:nn.Linear):
...
def channel_parallel_compute(input:torch.tensor, layer:nn.Conv2d, shift_mat, T=4, force_ann=False):
    input_hardware = input.clone().detach()
    # 确认 input 的时间步维度
    t_dimention=None
    ...
    # New Logic：decompose 6x6 convolution into multiple 3x3 convolutions, and handle them sequentially.
    if (k_h, k_w) == (6, 6):
        conv_3x3_1 = nn.Conv2d(in_channels=c_in, out_channels=c_out, kernel_size=(3, 3), stride=s, padding=0, dilation=dilation, groups=groups, bias=False)
        conv_3x3_2 = nn.Conv2d(in_channels=c_in, out_channels=c_out, kernel_size=(3, 3), stride=s, padding=0, dilation=dilation, groups=groups, bias=False)
        conv_3x3_3 = nn.Conv2d(in_channels=c_in, out_channels=c_out, kernel_size=(3, 3), stride=s, padding=0, dilation=dilation, groups=groups, bias=False)
        conv_3x3_4 = nn.Conv2d(in_channels=c_in, out_channels=c_out, kernel_size=(3, 3), stride=s, padding=0, dilation=dilation, groups=groups, bias=False)

        conv_3x3_1.weight.data = conv_weights[:, :, 0:3, 0:3]
        conv_3x3_2.weight.data = conv_weights[:, :, 0:3, 3:6]
        conv_3x3_3.weight.data = conv_weights[:, :, 3:6, 0:3]
        conv_3x3_4.weight.data = conv_weights[:, :, 3:6, 3:6]

        input_decompose_1 = input_hardware[:, :, 0:H-3, 0:W-3]
        input_decompose_2 = input_hardware[:, :, 0:H-3, 3:W]
        input_decompose_3 = input_hardware[:, :, 3:H, 0:W-3]
        input_decompose_4 = input_hardware[:, :, 3:H, 3:W]

        # Recursively solve the problem!
        shift_mat_1, shift_mat_2, shift_mat_3, shift_mat_4 = shift_mat
        output_1 = channel_parallel_compute(input_decompose_1, conv_3x3_1, shift_mat_1, T, force_ann)
        output_2 = channel_parallel_compute(input_decompose_2, conv_3x3_2, shift_mat_2, T, force_ann)
        output_3 = channel_parallel_compute(input_decompose_3, conv_3x3_3, shift_mat_3, T, force_ann)
        output_4 = channel_parallel_compute(input_decompose_4, conv_3x3_4, shift_mat_4, T, force_ann)
        ...
        if bias is not None:
            ...
            output += bias
        ## If kernel is 6x6, then don't go deeper. The output is returned here.
        return output
    ...
    if groups == 1:
        ...
        for t in range(len(kernel_num_queue)):
            kernel_num = kernel_num_queue[t]
            if (k_h, k_w) == (3, 3):
                kernel_list = [conv_weights[j, 0, :, :].view(k_h,k_w).to(torch.int64).detach().cpu().numpy() for j in range(c_out)]
                A2_list = [conv_weights[j,0,:,:].view(k_h,k_w).to(torch.int64).detach().cpu().numpy() for j in range(c_out)]
            elif (k_h, k_w) == (1, 3):
                kernel_list = []
                A2_list = []
                for j in range(c_out):
            ...
            input_list = [np.vectorize(lambda v: format(int(v) & 0xF, "04b"))(input_hardware[0,j,:,:].view(H,W)) for j in range(c_out)]
            dilation_list = [f'{d_h}*{d_w}' for j in range(c_out)]
            step_mode_list = [f"{T}" for i in range(c_out)]
            scale_list = [shift for j in range(c_out)]
            pe_mode_list = [mode for j in range(c_out)]
            a1_height = H
            stride = s
            depth = 1
            padding_one_dilated = 0
...
def dconv_hardware_compute(input:torch.tensor, layer:nn.ConvTranspose2d, shift_mat, T=4, force_ann=False):
    # 1. 获取参数
    t_dimension = None
    ...
    output = channel_parallel_compute(padded_input, conv_layer, shift_mat, T, force_ann)
...
if __name__ == '__main__':
    ...
    if args.test_project == 'conv':
        ...
        hardware_output = channel_parallel_compute(input, conv_layer, shift, T=4, force_ann=False)
        ...
        hardware_output = dconv_hardware_compute(random_input, dconv_layer)
