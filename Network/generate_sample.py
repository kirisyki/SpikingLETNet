...
if __name__ == '__main__':
    ...
    sample_channel = accumulated_conv_output[0, 0, :, :].view(1,1,400,400)
    # sample_channel = init_conv_output[0, 0, 0, :, :].view(1,1,400,400)
    min_val = torch.min(sample_channel)
    max_val = torch.max(sample_channel)
    print(f"activation min_val: {min_val} max_val: {max_val}")
    print(f"sample_channel_shape: {sample_channel.shape}")

    sample_conv = torch.nn.Conv2d(1, 1, kernel_size=3, stride=1, padding=1, bias=False).cuda()
    sample_conv.weight.data = torch.tensor(np.array([[1,0,2],
                                                     [1,0,2],
                                                     [0,0,1]]),dtype=torch.float32).view(1,1,3,3).cuda()
    with torch.no_grad():
        sample_output = sample_conv(sample_channel)
    sample_output_flat = sample_output.detach().cpu().numpy().flatten()
    plt.hist(sample_output_flat, bins=50)
    plt.savefig('sample_output.png', dpi=600)

    # first_conv_weight = model_q.init_conv[0].conv.layer.weight.data
    # print(f"first_conv_weight_shape: {first_conv_weight.shape}")
    # print(f"sample_conv_weight:{first_conv_weight[0,0,:,:]}")
    
    # 保存 t = 4
    npy_save_folder = '/home/wyl/projects/LETNet/hardware_sample/t4_sample'
    sample_channel_path = os.path.join(npy_save_folder, 'sample_channel.npy')
    sample_kernel_path = os.path.join(npy_save_folder, 'sample_kernel.npy')
    sample_output_path = os.path.join(npy_save_folder, 'sample_output.npy')
    np.save(sample_channel_path, sample_channel.detach().cpu().numpy())
    np.save(sample_kernel_path, sample_conv.weight.data.detach().cpu().numpy())
    np.save(sample_output_path, sample_output.detach().cpu().numpy())