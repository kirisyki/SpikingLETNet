...
class STransBlock(nn.Module):
    def __init__(
            self, n_feat=32, dim=288, num_heads=8, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
            drop_path=0., act_layer=nn.ReLU, norm_layer=nn.LayerNorm):
        super(STransBlock, self).__init__()
        self.dim = dim
        self.atten = EffAttention(self.dim, num_heads=8, qkv_bias=False, qk_scale=None,
                                  attn_drop=0., proj_drop=0.)
        self.norm1 = nn.LayerNorm(self.dim)
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1)
        x = x + self.atten(self.norm1(x))
        # mid = self.atten(self.norm1(x))
        # mid_mean = mid.mean().item()
        # mid_std = mid.std().item()
        # print(f"mid_mean: {mid_mean} mid_std: {mid_std}")
        x = x + self.mlp(self.norm2(x))
        # mean_y = x.mean().item()
        # std_y = x.std().item()
        # print(f"input_mean: {mean_x} input_std: {std_x} mid_mean: {mean_mid} mid_std: {std_mid} output_mean: {mean_y} output_std: {std_y}")
        return x