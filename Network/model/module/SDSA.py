...
class SDSA1(nn.Module):
    def __init__(
        ...
        self.q_conv = nn.Sequential(RepConv(dim, hid_dim, bias=False), nn.BatchNorm2d(hid_dim))
        self.k_conv = nn.Sequential(RepConv(dim, hid_dim, bias=False), nn.BatchNorm2d(hid_dim))
        self.v_conv = nn.Sequential(RepConv(dim, hid_dim, bias=False), nn.BatchNorm2d(hid_dim))
        ...
        self.proj_conv = nn.Sequential(
            RepConv(hid_dim, dim, bias=False), nn.BatchNorm2d(dim)
        )
    ...
    def forward(self, x):
        ...
        x = x.transpose(2, 3).reshape(B, C, H, W).contiguous()        
        x = self.proj_conv(x)
...
class SDSA2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(3, 4).reshape(B, C, H, W).contiguous()
        x = self.proj_conv(x)
