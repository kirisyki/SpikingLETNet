...
class DownSamplingBlock(nn.Module):
    def __init__(self, nIn, nOut):
        ...
        self.avg_pool = layer.AvgPool2d(2, stride=2)
        # self.bn_prelu = BNPReLU(nOut)
...
class SpikingLETNet(nn.Module):
    def __init__(self, classes=19, block_1=3, block_2=12, block_3=12, block_4=3, block_5 = 3, block_6 = 3):
        ...
        self.init_conv = nn.Sequential(
            ...
            Conv(32, 64, 3, 1, padding=1, bn_acti=True),
        ...
        for i in range(0, block_1):
            self.DAB_Block_1.add_module("DAB_Module_1_" + str(i), DABModule(128, d=2))
...
#DECODER
        ...
        self.upsample_1 = UpsampleingBlock(64, 32)
        ...
        self.LC1 = LongConnection(128, 32, 3, bn_acti=True)
        self.LC2 = LongConnection(128, 32, 3, bn_acti=True)
        self.LC3 = LongConnection(64, 64, 3, bn_acti=True)
...
if __name__ == '__main__':
    ...
    model = SpikingLETNet(classes=19).to(device)
    weight1 = model.state_dict()['DAB_Block_2.DAB_Module_2_0.ddconv3x1.conv.weight']
    weight2 = torch.load('/root/LETNet/Network/checkpoint/voc/SpikingLETNetbs32gpu1_trainval/model_501.pth')['model']['DAB_Block_2.DAB_Module_2_0.ddconv3x1.conv.weight']
    dist = torch.norm(weight1 - weight2, p=2)
    print("L2 distance:", dist.item())
    cos_sim = torch.nn.functional.cosine_similarity(
        weight1.flatten(), weight2.flatten(), dim=0
    )
    print("Cosine similarity:", cos_sim.item())
    # device = torch.device("cpu")
    # model = SpikingLETNet(classes=19).to(device)
    # model_str = str(model)
    # conv_lines = [line for line in model_str.splitlines() if "Conv" in line]
    # with open("conv_layers.txt", "w") as f:
    #     f.write("\n".join(conv_lines))
    # summary(model, (3, 512, 1024), device="cpu")
    # t = torch.rand([4, 3, 512, 1024])
    # print(model(t))
