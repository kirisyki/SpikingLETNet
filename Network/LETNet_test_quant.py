...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str,default="QAT_checkpoint/udd/SpikingLETNet_shallow_middlebs32gpu1_trainval20260104-185754/model_best.pth",
...
def test_model(args):
    ...
    datas, testLoader = build_dataset_test(args.dataset, args.num_workers, batch_size=args.batch_size)
    ...
    if not args.best:
        ...
        #         model_q.load_state_dict(torch.load(args.checkpoint_q, map_location=f'cuda:{args.gpus}')['model'])
        #     elif args.checkpoint_q.endswith('.pt'):
        #         print("=====> loading quantized checkpoint '{}'".format(args.checkpoint_q))
        #         model_q = torch.load(args.checkpoint_q, weights_only=False, map_location=f'cuda:{args.gpus}')
        ...
        set_computing_mode(model_q, 'pytorch')
