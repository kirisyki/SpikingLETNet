...
from quantization.int4_selfbuild import quantize_model, plot_scale_x
...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str, default="/home/wyl/projects/LETNet/checkpoint/udd/SpikingLETNet_shallow_middlebs64gpu1_trainval20260104-145425/model_best.pth")
...
def train_model(args):
    ...
    if args.config is not None:
        ...
        with open(args.savedir + os.path.basename(args.config), "w") as f:
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if args.only_save_best:
            ...
            elif args.dataset == 'cityscapes' or args.dataset ==  'voc':
...
def train_qat(args, train_loader, model, model_q, criterion, optimizer, epoch, temperature=0.01):
    """
    args:
    ...
    for iteration, batch in enumerate(train_loader, 0):
        ...
        output, mid_outputs = model.forward_qat(images)
        output_q, mid_outputs_q = model_q.forward_qat(images)
        output = output.mean(0)  # average on time steps
        output_q = output_q.mean(0)
        for i in range(len(mid_outputs)):
            mid_outputs[i] = mid_outputs[i].mean(0)
            mid_outputs_q[i] = mid_outputs_q[i].mean(0)
        # 计算 kd loss
        kd_loss = 0
        for o, oq in zip(mid_outputs, mid_outputs_q):
            kd_loss += kd_criterion(o, oq.detach())

        loss = criterion(output, labels) + kd_loss * temperature
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        print('=====> epoch[%d/%d] iter: (%d/%d) \tcur_lr: %.6f loss: %.3f kd_loss: %.3f time:%.2f' % (epoch + 1, args.max_epochs,
                                                                                         iteration + 1, total_batches,
                                                                                         lr, loss.item(), kd_loss.item(), time_taken))
