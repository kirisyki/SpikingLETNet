...
def test_model(args):
    ...
    if not args.best:
        ...
        with open('model_structure_q.txt', 'w') as f:
            f.write(str(model_q))
        ...
        for i in range(3):
            qlayer = model_q.init_conv[i].conv
            qlayer.hardware_computing = False
        print("=====> beginning validation")
        print("validation set length: ", len(testLoader))
        mIOU_val, per_class_iu = test(args, testLoader, model_q)
        from mid_activations_statics import mid_activations_dict
        torch.save(mid_activations_dict, 'mid_activations_dict.pth')
